 ...
 * <pre>
       <aspectj>
           <aspects>
               <aspect name="org.kaazing.aeron.debug.LoggingSlf4jAspect"/>
           </aspects>
           <weaver options="-verbose">
               <include within="uk.co.real_logic.aeron..*"/>
               <include within="org.kaazing.aeron.debug.LoggingSlf4jAspect"/>
           </weaver>
       </aspectj>
</pre>

 * The following jars must be on the classpath: aeron-logging-slf4j-VERSION.jar:aspectjweaver-VERSION.jar:aspectjrt-VERSION.jar
 * and the following java agent must be specified on the command line: -javaagent:path\to\aspectjweaver-VERSION.jar
 */
@Aspect
public class LoggingSlf4jAspect
{
    private static final Logger LOGGER = LoggerFactory.getLogger(LoggingSlf4jAspect.class);
    ...
    {    
        if(LOGGER.isTraceEnabled())
        {
            ...
            LOGGER.trace("{} ", joinPoint, sb.toString());
        }   
    }
}